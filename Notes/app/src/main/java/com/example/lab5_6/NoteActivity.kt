...
class NoteActivity(private var navController : NavHostController, private val noteId: String ) {
    ...
    private val notes = mutableStateOf(listOf<NotesViewModel>()) // Инициализация массива заметок
    ...
    init {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (noteId != "-1") { // Если noteId != "-1", значит заметка существует
                val noteFuture: CompletableFuture<Response> =
                    api.get("/GetNoteById?id=$noteId") // Запрос заметки по ID
                val noteResponse = noteFuture.get() // Ожидание ответа

                if (noteResponse.isSuccessful) {
                    val note = json.readValue<NotesViewModel>(
                        noteResponse.body?.string().toString()
