...
class APISender {
    val domain = "http://192.168.71.123:4000"
    ...
    fun get(url: String): CompletableFuture<Response>{
        ...
        val request = Request.Builder().url("$domain$url").build()
    ...
    fun post(url:String, json: String = ""):CompletableFuture<String>{
        ...
        val request = Request.Builder().url("$domain$url").header("Cache-Control", "no-cache")
            .addHeader("Content-Type", "application/json").addHeader("Accept", "*/*")
            .addHeader("Accept-Encoding", "gzip, deflate, br")
            .addHeader("Connection", "keep-alive").post(requestBody).build()
